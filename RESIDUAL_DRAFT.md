# RESIDUAL_DRAFT — reuse / approximate / recompute (분리 repo 구성)

**이 repo(residual_draft_dropin)는 본 repo(Frozen-flux-draft-DiT)와 완전히
분리된 git 프로젝트다.** 본 repo는 한 줄도 수정하지 않는다. 모든 신규 코드는
고유 최상위 패키지 `resdraft/` 아래에 있어 이름 충돌이 없고, 두 repo를
PYTHONPATH에 같이 올려서 쓴다:

```bash
export PYTHONPATH=$HOME/residual_draft_dropin:$HOME/Frozen-flux-draft-DiT
```

- 학습/진단: `python -m resdraft.training.<script>` (어디서 실행하든 무관)
- 본 repo sampler + error-head router: `python run_pivot.py ...`
  — repo의 `cached_flux_fill.main()`을 그대로 실행하되, 실행 직전에
  `RouterDraft.load`를 auto-detect factory로 monkeypatch한다
  (residual-draft ckpt(model_config 키) -> ErrorHeadRouter, legacy ckpt ->
  원본 RouterDraft; 재귀 없음). **상대경로(data/, out/)가 repo 기준이므로
  `cd ~/Frozen-flux-draft-DiT`에서 실행**하거나 절대경로를 쓴다.
...
python -m resdraft.training.train_residual_draft --teacher $TEACH --out $CKPT \
...
python -m resdraft.training.eval_residual_router --teacher $TEACH --ckpt $CKPT/last.pt

# 3) Stage 3: draft_only — reuse_c2+tail(8.9s, 0.0469)과 동일 target-eval 수.
#    성공 기준: mask-LPIPS→ref < 0.0469 (draft_ms 실측치 함께 보고)
python -m resdraft.samplers.three_tier_flux_fill --manifest $MAN --out out/stage_res \
...
python -m resdraft.samplers.three_tier_flux_fill ... --cache-period 5 --method draft_only
```
...
python -m resdraft.training.diagnose_transition --teacher $TEACH --shards 6
...
python -m resdraft.training.diagnose_residual --teacher $TEACH --ckpt $CKPT/last.pt
#   기대: offset-1 dz-identity rel err ~1e-3 이하 (bf16 덤프 오차 수준),
#         offset-1 ratio ≈ 1.0, offset-2도 ≈ 1.0 (콘텐츠 없이는 못 배움)

# 1) v2 재학습 (새 out 디렉토리)
python -m resdraft.training.train_residual_draft --teacher $TEACH     --out ${CKPT}_v2 --steps 60000 --cache-periods 2 3
...
인터페이스이고, `run_pivot.py`가 checkpoint를 자동 감지한다 (repo 무수정) (model_config 키 있으면 ErrorHeadRouter, 없으면 기존 CNN router).

```bash
# headline operating point 재측정: router := error head (3 seeds, 순차 실행)
# ckpt는 v2 smoke(spearman 0.75) 또는 heads 전용 재학습본 아무거나
cd ~/Frozen-flux-draft-DiT   # 상대경로 기준
for SEED in 0 1 2; do
python ~/residual_draft_dropin/run_pivot.py --manifest data/coco_manifest_1024.json \
